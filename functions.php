...
function twentytwelve_body_class($classes)
{
    ...
    if (!is_active_sidebar('sidebar-1') || is_page_template('page-templates/product-cat.php'))
        $classes[] = 'full-width';
...
function show_childpages_shortcode()
{
    ...
    if (is_page()) {
        ...
        if ($child_pages) {
            ...
            echo '<div class="container product-list"><div class="row">';

            foreach ($child_pages as $child_page) {

                $page_id    = $child_page->ID;
                $page_link  = get_permalink($page_id);
                $sku        = get_field('sku', $page_id);
                $page_img   = get_the_post_thumbnail($page_id, 'thumbnail');


                if (strlen($page_img) < 1) {
                    echo $page_img;
                    continue;
                }
